...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (k.openai) {
        setScreen('chat');
        setMessages([{ id: 1, role: 'ai', text: '안녕하세요! 대대행사 질문을 보내주세요. 매체 정책·광고 운영 기준 등 무엇이든 답변드립니다.', sources: [] }]);
      }
    }
  }, []);
  ...
  const send = async () => {
    ...
    try {
      ...
      const history = messages.filter(m => m.role !== 'ai' || m.id !== 1).slice(-6).map(m => ({
      ...
      setMessages(prev => [...prev, { id: Date.now(), role: 'ai', text: clean, sources }]);
    } catch (e) {
    ...
    <div style={S.setupWrap}>
      <div style={S.setupCard}>
        ...
        <div style={S.fieldWrap}>
          <label style={S.label}>
            Tavily API Key
            <span style={S.opt}>선택 · 실시간 검색</span>
          </label>
          <input
            style={S.input}
            type="password"
...
const S = {
  ...
  textarea:   {
    flex:1, background:'#161616', border:'1px solid #252525', borderRadius:12,
    color:'#e0e0e0', fontSize:13.5, fontFamily:'Noto Sans KR, sans-serif',
    padding:'10px 14px', resize:'none', outline:'none', lineHeight:1.6,
